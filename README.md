...
6. **Google Maps parser** — `POST /api/parser/run` (JWT): body `{ "campaign_id", "location", "keyword", "limit" }`. Returns immediately `{"status":"started"}`; Playwright opens Maps in the background, collects business cards, calls Claude for `pain_point`, tries to scrape email from websites (httpx + BeautifulSoup), saves leads with `source="parser"` and status `new`. Rows without a found email are skipped. Requires Playwright browsers (`python -m playwright install chromium`) and preferably `ANTHROPIC_API_KEY` (otherwise a fallback pain text is used).

If the database existed before `leads.source` was added, run in Postgres:
